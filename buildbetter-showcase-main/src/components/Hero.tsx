          ...
          <div className="max-w-3xl">
            <div className="mb-6">
              <span className="inline-block px-4 py-2 bg-accent/20 text-accent font-semibold rounded-full text-sm mb-4">
                {content?.badge ?? "🌍 Ubuntu Building Excellence"}
              </span>
            </div>
            
            <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold text-white mb-6 leading-tight">
              {content?.titleLine1 ?? "Ubuntu Building,"}
              <span className="text-accent block">{content?.titleLine2 ?? "African Excellence"}</span>
            </h1>
            
            <p className="text-xl md:text-2xl text-white/90 mb-8 leading-relaxed max-w-2xl">
              {content?.subtitle ?? "Leading African construction and trading company, proudly serving communities across South Africa with integrity, quality, and traditional Ubuntu values."}
            </p>
